...
from kinova_apps.button_press_action import ButtonPressAction
from kinova_apps.slider_action import SliderAction
from kinova_apps.plug_remove_slid_action import PlugRemoveSlidAction
from kinova_apps.probe_action import ProbeAction
...
class TaskSM(object):
    def __init__(self):
        ...
        self.board_detector_event_out = rospy.Subscriber(
            "~board_detector_event_out",
            std_msgs.msg.String,
            self.board_detector_event_cb,
        )
        self.board_detector_event_in = rospy.Publisher(
            "~board_detector_event_in", std_msgs.msg.String, queue_size=1
        )
        self.poi_event_in = rospy.Publisher(
            "~poi_event_in", std_msgs.msg.String, queue_size=1
        )
        self.init_board_pose_sub = rospy.Subscriber(
            "~init_approximate_board_pose",
            geometry_msgs.msg.PoseStamped,
            self.board_pose_cb,
        )
        self.perceive_board_pose = rospy.get_param(
        ...
        self.action_executors = [
            ButtonPressAction(self.arm, self.tu),
            ...
            ButtonPressAction(
                self.arm, self.tu, reference_frame="red_button_link"
            ),
        ]
    ...
    def run(self):
        ...
        for task_id in self.task_order:
            ...
            if not success:
                rospy.logerr(
                    "%s action failed"
                    % self.action_executors[task_id].__class__.__name__
                )
    ...
    def orient_arm_to_board(self):
        ...
        feedback = rospy.wait_for_message(
            "/my_gen3/base_feedback", BaseCyclic_Feedback
        )
